...
echo.
echo Dove vuoi salvare i risultati?
...
if "%scelta%"=="1" (
    ...
    echo Salvato in %output_txt%
) else if "%scelta%"=="2" (
    more +1 "%temp_csv%" >> "%output_csv%"
    echo Salvato in %output_csv%
) else (
...
if defined hostname (
    ...
    if not defined dns_error set "dns_error=ERRORE DNS non rilevato"
    echo %ip% = !dns_error! >> "%temp_file%"
    echo %ip%,NESSUN NOME TROVATO,%timestamp%,!dns_error! >> "%temp_csv%"
)
